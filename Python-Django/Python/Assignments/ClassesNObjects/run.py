...
def run_student_tests(target_folder, total_points):
...
class Submission():
    ...
    def submit_assignment(self):
        ...
        view_files            = self.config['view_file']
        for mf in modifiable_files:
            if not os.path.exists(mf):
                logging.error("Required assignment file not found: ")
        ...
        if r.status_code == 200:
            ...
            logging.error("="*80)      
            logging.error("It is taking a bit time so check your result in submission history on assessments.edyoda.com")      
            logging.error("="*80)
            sys.exit(1)
